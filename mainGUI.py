...
class InputFrame(ttk.Frame):
    ...
    def __create_widgets(self, valider, billard):
        ...
        ttk.Label(self, text="Type de billard choisi :").grid(column=0, row=0)
        ttk.Label(self, text="Paramètres de frappe :").grid(column=0, row=5)
        ttk.Label(self, text="Angle de frappe :").grid(column=0, row=6)
        ttk.Label(self, text="Force de frappe :").grid(column=0, row=7)

        # Création des entrées
        self.choix = tk.IntVar()
        self.choix.set(1)
        self.choix1_entry = ttk.Radiobutton(self, text="Français", variable=self.choix, value=1)
        self.choix1_entry.grid(column=1, row=1)
        self.choix2_entry = ttk.Radiobutton(self, text="Américain", variable=self.choix, value=2)
        self.choix2_entry.grid(column=1, row=2)
        self.choix3_entry = ttk.Radiobutton(self, text="Anglais", variable=self.choix, value=3)
        self.choix3_entry.grid(column=1, row=3)
        self.choix4_entry = ttk.Radiobutton(self, text="Personnalisé", variable=self.choix, value=4)
        self.choix4_entry.grid(column=1, row=4)

        self.angle_entry = tk.Scale(self, from_=-180, to=180, orient="horizontal", length=150, tickinterval=90, resolution=1)
        self.angle_entry.grid(column=1, row=6)

        self.force_entry = tk.Scale(self, from_=0, to=100, orient="horizontal", length=150, tickinterval=25, resolution=1)
        self.force_entry.grid(column=1, row=7)
